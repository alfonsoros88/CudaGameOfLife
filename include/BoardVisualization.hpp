...
class BoardVisualization {
    public:
        BoardVisualization (const size_t width, const size_t height, const size_t radius) 
            ...
            img = cv::Mat::zeros(2 * r * w, 2 * r * h, CV_8UC1);
    ...
    private:
        ...
        void boardToImage(const cv::Mat& board, cv::Mat& image) {
            for (int i = 0, x = r; i < w; i++, x += 2 * r) {
                for (int j = 0, y = r; j < h; j++, y += 2 * r) {
                    cv::Scalar color(board.at<unsigned char>(i, j) ? 255 : 0);
                    cv::circle(image, cv::Point2i(x, y), r, color, CV_FILLED, 8, 0);
                }
            }
        }
};
